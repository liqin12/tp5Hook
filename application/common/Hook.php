...
class Hook
{
    ...
    static public function call($type , $name , &$array = array())
    {
        ...
        $ret = '';
        //获得config里面钩子设置模式，如果有调用think系统钩子进行监听
        if(Config::get('jntoo_hook_call')){
            ...
            if(isset(self::$hookList[$key]))
            {
                foreach(self::$hookList[$key] as $r){
                    // 闭包处理
                    $result = '';
                    if(is_callable($r)){//is_callable是否为合法的可调用结构
                        $result = call_user_func_array($r, $array);//call_user_func_array----调用回调函数，并把一个数组参数作为回调函数
                    }elseif(is_object($r)){
                        // 自己定义对象钩子
